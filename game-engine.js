...
class LiarDiceSession {
  constructor(options = {}) { this.dicePerPlayer = Math.max(2, Math.min(8, Number(options.dice || 5))); this.playerScore = 0; this.aiScore = 0; this.round = 0; this.start(); }
  ...
  validate(quantity, face) { if (quantity < 1 || quantity > this.dicePerPlayer * 2 || face < 1 || face > 6 || !this.higher([quantity, face])) throw new Error("bid must be higher than the public bid"); return [quantity, face]; }
  probability(bid) { if (!bid) return null; const [quantity, face] = bid; const own = this.player.filter(value => value === face || (face !== 1 && value === 1)).length; const needed = quantity - own; if (needed <= 0) return 1; const p = face === 1 ? 1/6 : 1/3; let total = 0; for (let k = Math.max(needed, 0); k <= this.dicePerPlayer; k += 1) { let c = 1; for (let i = 1; i <= k; i += 1) c = c * (this.dicePerPlayer - i + 1) / i; total += c * p ** k * (1-p) ** (this.dicePerPlayer-k); } return total; }
