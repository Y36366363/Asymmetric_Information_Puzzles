...
const GAMES = [
  ...
  ["goofspiel", "秘密竞价", "奖牌逐轮揭晓，双方同时秘密竞价；在直觉型与精确均衡 AI 之间切换挑战。", "单人 · 同时行动与秘密竞价", true],
...
const SECURITY_HEADERS = {
  "content-security-policy": "default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self' data:; connect-src 'self'; base-uri 'none'; form-action 'self'; object-src 'none'; frame-ancestors 'none'",
...
class ECardSession {
  constructor(){this.round=0;this.playerScore=0;this.aiScore=0;this.timings={emperor:[],slave:[]};this.roundHistory=[];this.start();}
  timingPolicy(){const timings=this.timings[this.playerRole].slice(-8);if(!timings.length)return["uniform",Array(5).fill(.2)];const counts=Array.from({length:5},(_,index)=>timings.filter(value=>value===index+1).length);let mode,weights;if(this.aiRole==="slave"){mode="chase_player_timing";weights=counts.map(count=>2+3*count);}else{mode="avoid_player_timing";const peak=Math.max(...counts);weights=counts.map(count=>2+3*(peak-count));}const total=weights.reduce((sum,value)=>sum+value,0);return[mode,weights.map(value=>value/total)];}
  sampleDuel(distribution){const draw=Math.random();let cumulative=0;for(let index=0;index<distribution.length;index+=1){cumulative+=distribution[index];if(draw<cumulative)return index+1;}return 5;}
  start(){this.round+=1;this.playerRole=this.round%2?"emperor":"slave";this.aiRole=this.playerRole==="emperor"?"slave":"emperor";this.player=[this.playerRole,"citizen","citizen","citizen","citizen"];this.ai=[this.aiRole,"citizen","citizen","citizen","citizen"];[this.aiPolicyMode,this.aiTimingDistribution]=this.timingPolicy();this.aiCommittedDuel=this.sampleDuel(this.aiTimingDistribution);this.phase="playing";this.history=[];this.last=null;this.result=null;}
  act(action,payload){if(action==="next_round"&&this.phase==="finished"){this.start();return;}const card=payload.card;if(action!=="play_card"||this.phase!=="playing"||!this.player.includes(card))throw new Error("illegal E-Card action");const duel=this.history.length+1,remaining=this.aiTimingDistribution.slice(duel-1).reduce((sum,value)=>sum+value,0),probability=this.aiTimingDistribution[duel-1]/remaining,aiCard=duel===this.aiCommittedDuel?this.aiRole:"citizen";this.player.splice(this.player.indexOf(card),1);this.ai.splice(this.ai.indexOf(aiCard),1);const wins={emperor:"citizen",citizen:"slave",slave:"emperor"};const outcome=card===aiCard?"draw":wins[card]===aiCard?"player":"ai";this.last={duel,playerCard:card,aiCard,outcome,aiSpecialProbability:probability};this.history.push(this.last);if(card===this.playerRole)this.timings[this.playerRole].push(duel);if(outcome!=="draw"){const role=outcome==="player"?this.playerRole:this.aiRole,points=role==="slave"?5:1,playerSpecial=this.history.find(item=>item.playerCard===this.playerRole)?.duel??null;if(outcome==="player")this.playerScore+=points;else this.aiScore+=points;this.result={winner:outcome,winnerRole:role,points,decisiveDuel:duel,aiCommittedDuel:this.aiCommittedDuel,playerSpecialDuel:playerSpecial};this.roundHistory.push({round:this.round,playerRole:this.playerRole,aiPolicyMode:this.aiPolicyMode,aiCommittedDuel:this.aiCommittedDuel,playerSpecialDuel:playerSpecial,winner:outcome,points});this.phase="finished";}}
  snapshot(){const hand=[...new Set(this.player)].map(card=>({card,count:this.player.filter(c=>c===card).length})),next=this.phase==="finished"?1:Math.min(this.history.length+1,5),remaining=this.aiTimingDistribution.slice(next-1).reduce((sum,value)=>sum+value,0),forecast=this.aiTimingDistribution.map((probability,index)=>({duel:index+1,probability:index+1<next?0:probability/remaining}));return{gameId:"e-card",phase:this.phase,roundNumber:this.round,duelNumber:this.history.length+(this.phase==="finished"?0:1),playerRole:this.playerRole,aiRole:this.aiRole,playerHand:hand,opponentCardsLeft:this.ai.length,playerScore:this.playerScore,aiScore:this.aiScore,history:this.history,roundHistory:this.roundHistory,lastReveal:this.last,result:this.result,aiPolicyMode:this.aiPolicyMode,aiTimingForecast:forecast,aiCommittedDuel:this.phase==="finished"?this.aiCommittedDuel:null,strategyScope:"precommitted_timing_with_bounded_cross_round_adaptation",strategyEvidence:"strong_heuristic",legalActions:[this.phase==="playing"?"play_card":"next_round"],informationSet:{privateHand:this.player,publicHistory:this.history,possibleOpponentCards:[...new Set(this.ai)].sort(),opponentCardsLeft:this.ai.length,aiTimingForecast:forecast,observedRoundHistory:this.roundHistory}};}
}
...
class RPSSession {
  ...
  act(action,payload){if(action==="new_match"&&this.phase==="finished"){this.start();return;}const move=payload.move;if(action!=="play_move"||this.phase!=="playing"||!this.player[move])throw new Error("illegal RPS action");const equilibrium=this.distribution(this.ai),observed=this.round,prior=this.distribution(this.player);const prediction=Object.fromEntries(Object.keys(this.player).map(k=>[k,.55*prior[k]+.45*(this.seen[k]+1)/(observed+3)]));const counters={rock:"paper",paper:"scissors",scissors:"rock"};const likely=Object.keys(prediction).sort((a,b)=>prediction[b]-prediction[a])[0],best=counters[likely],weight=Math.min(.32,observed*.045);let final=Object.fromEntries(Object.keys(equilibrium).map(k=>[k,(1-weight)*equilibrium[k]+(k===best?weight:0)]));const sum=Object.values(final).reduce((a,b)=>a+b,0);final=Object.fromEntries(Object.entries(final).map(([k,v])=>[k,v/sum]));const aiMove=sample(final);this.player[move]-=1;this.ai[aiMove]-=1;this.seen[move]+=1;this.round+=1;const beats={rock:"scissors",paper:"rock",scissors:"paper"};const outcome=move===aiMove?"draw":beats[move]===aiMove?"player":"ai";if(outcome==="draw")this.draws+=1;else if(outcome==="player")this.playerScore+=1;else this.aiScore+=1;const analysis={equilibriumDistribution:equilibrium,playerMinimaxDistribution:prior,predictedPlayerDistribution:prediction,bestResponse:best,exploitWeight:weight,finalDistribution:final,minimaxValue:0};this.history.push({round:this.round,playerMove:move,aiMove,outcome,analysis});this.last=analysis;if(Object.values(this.player).every(v=>v===0))this.phase="finished";}
  review(){if(this.phase!=="finished")return null;const probabilities=this.history.map(item=>item.analysis.playerMinimaxDistribution[item.playerMove]),counts=Object.fromEntries(Object.keys(this.player).map(move=>[move,this.history.filter(item=>item.playerMove===move).length])),peak=Math.max(...Object.values(counts));return{scoreDifference:this.playerScore-this.aiScore,equilibriumSupportedRounds:probabilities.filter(value=>value>1e-9).length,averageChosenProbability:probabilities.reduce((a,b)=>a+b,0)/probabilities.length,mostUsedMoves:Object.keys(counts).filter(move=>counts[move]===peak),moveCounts:counts,maxExploitWeight:Math.max(...this.history.map(item=>item.analysis.exploitWeight))};}
  snapshot(){return{gameId:"restricted-rps",phase:this.phase,roundNumber:this.round,roundsTotal:this.copies*3,playerInventory:this.player,aiInventory:this.ai,playerScore:this.playerScore,aiScore:this.aiScore,draws:this.draws,history:this.history,lastAnalysis:this.last,postMatchReview:this.review(),equilibriumRecommendation:this.distribution(this.player),legalActions:[this.phase==="playing"?"play_move":"new_match"],informationSet:{privateChoice:null,publicHistory:this.history,knownInventories:{player:this.player,ai:this.ai}}};}
}

class BlackjackSession {
  constructor(){this.bankroll=0;this.wins=0;this.losses=0;this.pushes=0;this.decisions=0;this.matches=0;this.practiceDecisions=0;this.practiceMatches=0;this.round=0;this.buildShoe();this.start();}
...
class LiarDiceSession {
  ...
  aiRespond() { const evaluatedBid=[...this.bid], confidence = this.probability(this.bid,this.ai); if (confidence < .45 || this.bid[0] >= this.dicePerPlayer * 2) { this.history.push({actor:"ai", action:"challenge", bid:this.bid, confidence}); this.resolve("ai"); return; } const preferred = [1,2,3,4,5,6].sort((a,b) => (this.ai.filter(v=>v===b).length + (b===1?0:this.ai.filter(v=>v===1).length)) - (this.ai.filter(v=>v===a).length + (a===1?0:this.ai.filter(v=>v===1).length)))[0]; const next = this.bid[0] < this.dicePerPlayer*2 ? [this.bid[0]+1, preferred] : [this.bid[0], this.bid[1]+1]; if (!this.higher(next) || next[1] > 6) { this.history.push({actor:"ai", action:"challenge", bid:this.bid, confidence}); this.resolve("ai"); return; } this.bid = next; this.history.push({actor:"ai", action:"raise", quantity:next[0], face:next[1], evaluatedBid, confidence}); this.turn = "player"; }
  ...
  snapshot() { const confidence = this.probability(this.bid), publicHistory=this.history.map(item=>Object.fromEntries(Object.entries(item).filter(([key])=>!["confidence","evaluatedBid"].includes(key)))), audit=this.phase==="finished"?this.history.filter(item=>item.actor==="ai"&&item.confidence!=null).map(item=>({action:item.action,bid:item.evaluatedBid||item.bid||[item.quantity,item.face],privateConfidence:item.confidence})):null; return {gameId:"liars-dice", phase:this.phase, roundNumber:this.round, dicePerPlayer:this.dicePerPlayer, playerDice:this.player, aiDice:this.phase === "finished" ? this.ai : null, opponentDiceCount:this.ai.length, currentBid:this.bid, minimumBid:this.bid ? {quantity:this.bid[1] < 6 ? this.bid[0] : this.bid[0]+1, face:this.bid[1] < 6 ? this.bid[1]+1 : 1} : null, turn:this.turn, playerScore:this.playerScore, aiScore:this.aiScore, claimProbability:confidence, history:publicHistory, postRoundDecisionAudit:audit, result:this.result, strategyScope:"private-dice threshold heuristic with post-round audit", strategyEvidence:"strong_heuristic", legalActions:this.phase === "bidding" && this.turn === "player" ? ["raise_bid","challenge"] : this.phase === "finished" ? ["new_round"] : [], informationSet:{privateHand:this.player, publicHistory, opponentDiceCount:this.ai.length, claimProbability:confidence}}; }
}
...
function battleHitClusters(hitKeys){const remaining=new Set(hitKeys),clusters=[];while(remaining.size){const seed=remaining.values().next().value;remaining.delete(seed);const cluster=[seed],frontier=[battleCell(seed)];while(frontier.length){const[row,column]=frontier.pop();for(const cell of[[row-1,column],[row+1,column],[row,column-1],[row,column+1]]){const key=battleKey(cell);if(remaining.delete(key)){cluster.push(key);frontier.push(cell);}}}const rows=new Set(cluster.map(key=>battleCell(key)[0])),columns=new Set(cluster.map(key=>battleCell(key)[1]));if(rows.size===1||columns.size===1)clusters.push(new Set(cluster));else for(const key of cluster.sort())clusters.push(new Set([key]));}return clusters;}
function battleDensity(tracker,size){const scores=new Map();for(let row=0;row<size;row+=1)for(let column=0;column<size;column+=1){const key=battleKey([row,column]);if(!tracker.shots.has(key))scores.set(key,0);}let candidatePlacements=0;const clusters=battleHitClusters(tracker.hits);for(const length of tracker.remaining){let candidates=battlePlacements(size,length).filter(cells=>cells.every(cell=>!tracker.misses.has(battleKey(cell))&&!tracker.sunk.has(battleKey(cell))));if(clusters.length&&size<=12){const focused=candidates.filter(cells=>{const keys=new Set(cells.map(battleKey));return clusters.some(cluster=>[...cluster].every(key=>keys.has(key)));});if(focused.length)candidates=focused;}else if(clusters.length){const focused=candidates.filter(cells=>cells.some(cell=>tracker.hits.has(battleKey(cell))));if(focused.length)candidates=focused;}candidatePlacements+=candidates.length;for(const cells of candidates)for(const cell of cells){const key=battleKey(cell);if(scores.has(key))scores.set(key,scores.get(key)+1);}}return{scores,candidatePlacements};}
function chooseBattleShot(tracker,size,randomize=true){const{scores,candidatePlacements}=battleDensity(tracker,size),peak=Math.max(...scores.values()),best=[...scores].filter(([,score])=>score===peak).map(([key])=>key).sort(),key=randomize?randomChoice(best):best[0];return{cell:battleCell(key),analysis:{candidatePlacements,peakDensity:peak,tiedBestCells:best.length,searchMode:tracker.hits.size?"target":"hunt",coverageShare:candidatePlacements?peak/candidatePlacements:0,chosenCell:battleCell(key)}};}
...
class BattleshipSession {
  ...
  configure(size){if(!BATTLE_FLEETS[size])throw new Error("boardSize must be 10, 12, or 15");this.size=size;this.lengths=[...BATTLE_FLEETS[size]];this.player=battleFleet(this.size,this.lengths);this.enemy=battleFleet(this.size,this.lengths);this.ai=battleTracker(this.lengths);this.advisor=battleTracker(this.lengths);this.phase="placement";this.turn=0;this.volley=1;this.playerShotsInVolley=0;this.winner=null;this.history=[];this.lastAiAnalysis=null;}
  salvoSize(){return this.size===15?2:1;}
  ...
  act(action,payload={}){if(action==="randomize_fleet"){if(this.phase!=="placement")throw new Error("fleet placement is already locked");this.player=battleFleet(this.size,this.lengths);return;}if(action==="set_board_size"){if(this.phase!=="placement")throw new Error("board size can only change during placement");const size=Number(payload.boardSize);if(!Number.isInteger(size)||!BATTLE_FLEETS[size])throw new Error("boardSize must be 10, 12, or 15");this.configure(size);return;}if(action==="rotate_ship"){if(this.phase!=="placement")throw new Error("ships can only rotate during placement");rotateBattleShip(this.player,Number(payload.shipId),this.size);return;}if(action==="start_battle"){if(this.phase!=="placement")throw new Error("battle has already started");this.phase="player_turn";return;}if(action!=="fire"||this.phase!=="player_turn")throw new Error("fire only when the battle is active");const cell=[Number(payload.row),Number(payload.column)],playerOutcome=battleFire(this.enemy,cell,this.size);observeBattle(this.advisor,playerOutcome);this.turn+=1;const event={turn:this.turn,volley:this.volley,playerShot:this.outcome(playerOutcome),aiShot:null,aiShots:[]};if(!this.remaining(this.enemy).length){this.phase="finished";this.winner="player";this.history.push(event);return;}this.playerShotsInVolley+=1;if(this.playerShotsInVolley<this.salvoSize()){this.history.push(event);return;}const analyses=[];for(let shot=0;shot<this.salvoSize();shot+=1){const decision=chooseBattleShot(this.ai,this.size),aiOutcome=battleFire(this.player,decision.cell,this.size);observeBattle(this.ai,aiOutcome);event.aiShots.push(this.outcome(aiOutcome));analyses.push(decision.analysis);if(!this.remaining(this.player).length){this.phase="finished";this.winner="ai";break;}}event.aiShot=event.aiShots[0]||null;this.lastAiAnalysis={...analyses.at(-1),volleyShots:analyses};this.history.push(event);this.playerShotsInVolley=0;this.volley+=1;}
  snapshot(){const density=battleDensity(this.advisor,this.size),suggestion=this.phase==="player_turn"?chooseBattleShot(this.advisor,this.size,false).cell:null,finished=this.phase==="finished";return{gameId:"battleship",phase:this.phase,turn:this.turn,volleyNumber:this.volley,salvoSize:this.salvoSize(),shotsRemainingInVolley:this.phase==="player_turn"?this.salvoSize()-this.playerShotsInVolley:0,winner:this.winner,boardSize:this.size,boardSizes:Object.keys(BATTLE_FLEETS).map(Number),shipLengths:this.lengths,fleet:this.player.ships.map((ship,id)=>({id,length:ship.length,orientation:battleOrientation(ship)})),playerBoard:this.boardPayload(this.player,true),enemyBoard:this.boardPayload(this.enemy,finished),playerShipsRemaining:this.remaining(this.player),enemyShipsRemaining:this.remaining(this.enemy),suggestedShot:suggestion,candidatePlacementCount:density.candidatePlacements,lastAiAnalysis:this.lastAiAnalysis,history:this.history,strategyScope:"cluster-consistent one-step placement density on 10x10/12x12; tail-protected legacy density on 15x15; not a full-fleet posterior optimum",legalActions:this.phase==="placement"?["randomize_fleet","set_board_size","rotate_ship","start_battle"]:this.phase==="player_turn"?["fire"]:[],informationSet:{misses:[...this.advisor.misses].map(battleCell),unresolvedHits:[...this.advisor.hits].map(battleCell),sunkCells:[...this.advisor.sunk].map(battleCell),remainingShipLengths:this.advisor.remaining,candidatePlacementCount:density.candidatePlacements,confirmedEnemyHits:this.advisor.hits.size+this.advisor.sunk.size,enemySegmentsTotal:this.lengths.reduce((a,b)=>a+b,0),searchedCells:this.advisor.shots.size,boardCells:this.size*this.size}};}
}
...
const PURSUIT_NODES={1:[8,14],2:[28,9],3:[50,8],4:[72,12],5:[91,20],6:[16,37],7:[38,31],8:[61,34],9:[82,40],10:[7,61],11:[29,57],12:[51,60],13:[74,63],14:[93,58],15:[19,84],16:[43,89],17:[69,86],18:[92,82]};
const PURSUIT_TAXI=[[1,2],[1,6],[2,3],[2,6],[2,7],[3,4],[3,7],[3,8],[4,5],[4,8],[4,9],[5,9],[6,7],[6,10],[6,11],[7,8],[7,11],[7,12],[8,9],[8,12],[8,13],[9,13],[9,14],[10,11],[10,15],[11,12],[11,15],[11,16],[12,13],[12,16],[12,17],[13,14],[13,17],[13,18],[14,18],[15,16],[16,17],[17,18]];
const PURSUIT_BUS=[[1,7],[2,8],[3,9],[4,7],[5,8],[6,12],[7,13],[8,14],[9,12],[10,16],[11,17],[12,18],[13,16],[14,17]];
const PURSUIT_EDGES=[...PURSUIT_TAXI.map(([from,to])=>({from,to,transport:"taxi"})),...PURSUIT_BUS.map(([from,to])=>({from,to,transport:"bus"}))];
const PURSUIT_GRAPH=Object.fromEntries(Object.keys(PURSUIT_NODES).map(node=>[Number(node),[]]));
for(const edge of PURSUIT_EDGES){PURSUIT_GRAPH[edge.from].push([edge.to,edge.transport]);PURSUIT_GRAPH[edge.to].push([edge.from,edge.transport]);}
function pursuitDistances(start){const distances={[start]:0},queue=[start];while(queue.length){const node=queue.shift();for(const[neighbor]of PURSUIT_GRAPH[node])if(distances[neighbor]===undefined){distances[neighbor]=distances[node]+1;queue.push(neighbor);}}return distances;}
const PURSUIT_DISTANCES=Object.fromEntries(Object.keys(PURSUIT_NODES).map(node=>[Number(node),pursuitDistances(Number(node))]));
class HiddenPursuitSession {
  constructor(){this.maxRounds=12;this.revealRounds=[3,6,9];this.gamesCompleted=0;this.detectiveWins=0;this.start();}
  start(){this.detectives=[1,18];this.fugitive=randomChoice(Object.keys(PURSUIT_NODES).map(Number).filter(node=>!this.detectives.includes(node)));this.belief=new Set(Object.keys(PURSUIT_NODES).map(Number).filter(node=>!this.detectives.includes(node)));this.round=1;this.currentDetective=0;this.phase="detective_turn";this.winner=null;this.lastTransport=null;this.lastReveal=null;this.history=[];this.recorded=false;}
  legalMoves(index=this.currentDetective){const occupied=this.detectives[1-index];return PURSUIT_GRAPH[this.detectives[index]].map(([node])=>node).filter(node=>node!==occupied).sort((a,b)=>a-b);}
  beliefAfter(mode){const next=new Set();for(const origin of this.belief)for(const[destination,edgeMode]of PURSUIT_GRAPH[origin])if(edgeMode===mode&&!this.detectives.includes(destination))next.add(destination);return next;}
  chooseFugitive(){const candidates=PURSUIT_GRAPH[this.fugitive].filter(([node])=>!this.detectives.includes(node)),scored=[];for(const[destination,mode]of candidates){const distance=Math.min(...this.detectives.map(detective=>PURSUIT_DISTANCES[destination][detective])),exits=PURSUIT_GRAPH[destination].filter(([node])=>!this.detectives.includes(node)).length,ambiguity=this.beliefAfter(mode).size;scored.push({destination,mode,score:distance*20+exits+ambiguity*.35});}const peak=Math.max(...scored.map(item=>item.score));return randomChoice(scored.filter(item=>item.score===peak));}
  finish(winner){this.phase="finished";this.winner=winner;this.belief=new Set([this.fugitive]);if(!this.recorded){this.gamesCompleted+=1;if(winner==="detectives")this.detectiveWins+=1;this.recorded=true;}}
  moveFugitive(){const legal=PURSUIT_GRAPH[this.fugitive].filter(([node])=>!this.detectives.includes(node));if(!legal.length){this.finish("detectives");return;}const move=this.chooseFugitive();this.fugitive=move.destination;this.lastTransport=move.mode;this.belief=this.beliefAfter(move.mode);const revealed=this.revealRounds.includes(this.round);if(revealed){this.belief=new Set([this.fugitive]);this.lastReveal=this.fugitive;}this.history.push({round:this.round,actor:"fugitive",from:null,to:revealed?this.fugitive:null,transport:move.mode,revealed});if(this.round>=this.maxRounds)this.finish("fugitive");else this.round+=1;}
  act(action,payload={}){if(action==="new_game"){this.start();return;}if(action!=="move_detective"||this.phase!=="detective_turn")throw new Error("detectives can move only during their turn");const destination=boundedInteger(payload.node,0,1,18,"node");if(!this.legalMoves().includes(destination))throw new Error("destination is not a legal detective move");const actor=this.currentDetective,origin=this.detectives[actor];this.detectives[actor]=destination;if(destination===this.fugitive){this.history.push({round:this.round,actor,from:origin,to:destination,capture:true});this.finish("detectives");return;}this.belief.delete(destination);this.history.push({round:this.round,actor,from:origin,to:destination,capture:false});if(actor===0)this.currentDetective=1;else{this.currentDetective=0;this.moveFugitive();}}
  snapshot(){const finished=this.phase==="finished";return{gameId:"hidden-pursuit",phase:this.phase,round:this.round,maxRounds:this.maxRounds,revealRounds:this.revealRounds,detectives:this.detectives,currentDetective:this.currentDetective,legalMoves:finished?[]:this.legalMoves(),belief:[...this.belief].sort((a,b)=>a-b),lastTransport:this.lastTransport,lastReveal:this.lastReveal,fugitivePosition:finished?this.fugitive:null,winner:this.winner,nodes:Object.entries(PURSUIT_NODES).map(([id,[x,y]])=>({id:Number(id),x,y})),edges:PURSUIT_EDGES,history:this.history,legalActions:finished?["new_game"]:["move_detective"],sessionStats:{gamesCompleted:this.gamesCompleted,detectiveWins:this.detectiveWins},informationSet:{possibleNodes:[...this.belief].sort((a,b)=>a-b),possibleCount:this.belief.size,lastPublicTransport:this.lastTransport,lastRevealedNode:this.lastReveal}};}
}

const GUESS_WHO_QUESTIONS=[
  {id:"hair_black",attribute:"hair",value:"black",label:"Does the person have black hair?"},
  {id:"hair_brown",attribute:"hair",value:"brown",label:"Does the person have brown hair?"},
  {id:"hair_blond",attribute:"hair",value:"blond",label:"Does the person have blond hair?"},
  {id:"hair_red",attribute:"hair",value:"red",label:"Does the person have red hair?"},
  {id:"glasses",attribute:"glasses",value:true,label:"Does the person wear glasses?"},
  {id:"hat",attribute:"hat",value:true,label:"Does the person wear a hat?"},
  {id:"facial_hair",attribute:"facialHair",value:true,label:"Does the person have facial hair?"},
  {id:"smiling",attribute:"smiling",value:true,label:"Is the person smiling?"},
];
const GUESS_WHO_NAMES=["Ada","Bruno","Cleo","Dante","Esme","Farah","Hugo","Iris","Jules","Kira","Leon","Mira","Nico","Opal","Pavel","Quinn","Rosa","Soren","Talia","Uma","Vik","Wren","Xavi","Yara"];
const GUESS_WHO_PATTERNS={black:[0,3,5,10,12,15],brown:[1,2,7,8,13,14],blond:[0,6,7,9,10,15],red:[1,4,6,9,11,14]};
const GUESS_WHO_CHARACTERS=[];
let guessWhoNameIndex=0;
for(const[hair,patterns]of Object.entries(GUESS_WHO_PATTERNS))for(const pattern of patterns){GUESS_WHO_CHARACTERS.push({name:GUESS_WHO_NAMES[guessWhoNameIndex++],hair,glasses:Boolean(pattern&1),hat:Boolean(pattern&2),facialHair:Boolean(pattern&4),smiling:Boolean(pattern&8)});}
const GUESS_WHO_FULL=(1<<GUESS_WHO_CHARACTERS.length)-1;
const GUESS_WHO_ALL_QUESTIONS=(1<<GUESS_WHO_QUESTIONS.length)-1;
const GUESS_WHO_YES_MASKS=GUESS_WHO_QUESTIONS.map(question=>GUESS_WHO_CHARACTERS.reduce((mask,character,index)=>character[question.attribute]===question.value?mask|(1<<index):mask,0));
const GUESS_WHO_EXPECTED_MEMO=new Map();
function guessWhoCount(mask){let count=0;for(let value=mask;value;value&=value-1)count+=1;return count;}
function guessWhoSplit(candidates,index){const yes=candidates&GUESS_WHO_YES_MASKS[index];return[yes,candidates&(~GUESS_WHO_YES_MASKS[index]&GUESS_WHO_FULL)];}
function guessWhoExpected(candidates,remaining){if(guessWhoCount(candidates)<=1)return 0;const key=`${candidates}:${remaining}`;if(GUESS_WHO_EXPECTED_MEMO.has(key))return GUESS_WHO_EXPECTED_MEMO.get(key);const total=guessWhoCount(candidates);let best=Infinity;for(let index=0;index<GUESS_WHO_QUESTIONS.length;index+=1){const bit=1<<index;if(!(remaining&bit))continue;const[yes,no]=guessWhoSplit(candidates,index);if(!yes||!no)continue;const next=remaining&~bit,value=1+guessWhoCount(yes)/total*guessWhoExpected(yes,next)+guessWhoCount(no)/total*guessWhoExpected(no,next);best=Math.min(best,value);}GUESS_WHO_EXPECTED_MEMO.set(key,best);return best;}
function guessWhoRecommendation(candidates,remaining){const total=guessWhoCount(candidates);if(total===1){const index=Math.log2(candidates);return{type:"guess",character:GUESS_WHO_CHARACTERS[index].name,projectedExpectedTurns:1,modelScope:"exact_fixed_roster_question_bank"};}let best=null;for(let index=0;index<GUESS_WHO_QUESTIONS.length;index+=1){const bit=1<<index;if(!(remaining&bit))continue;const[yes,no]=guessWhoSplit(candidates,index);if(!yes||!no)continue;const yesCount=guessWhoCount(yes),noCount=guessWhoCount(no),next=remaining&~bit,future=yesCount/total*guessWhoExpected(yes,next)+noCount/total*guessWhoExpected(no,next),key=[future,Math.max(yesCount,noCount),index];if(!best||key.some((value,keyIndex)=>value<best.key[keyIndex]&&key.slice(0,keyIndex).every((prior,i)=>prior===best.key[i]))){best={type:"question",questionId:GUESS_WHO_QUESTIONS[index].id,label:GUESS_WHO_QUESTIONS[index].label,yesCount,noCount,worstRemaining:Math.max(yesCount,noCount),expectedRemaining:(yesCount*yesCount+noCount*noCount)/total,projectedExpectedTurns:1+future+1,modelScope:"exact_fixed_roster_question_bank",key};}}delete best.key;return best;}
class GuessWhoSession{
  constructor(){this.maxTurns=8;this.gamesCompleted=0;this.gamesWon=0;this.totalWinningTurns=0;this.bestTurns=null;this.start();}
  start(){this.secret=Math.floor(Math.random()*GUESS_WHO_CHARACTERS.length);this.candidates=GUESS_WHO_FULL;this.remaining=GUESS_WHO_ALL_QUESTIONS;this.turns=0;this.phase="playing";this.history=[];this.result=null;}
  finish(won,reason){this.candidates=1<<this.secret;this.phase="finished";this.result={won,reason,secret:GUESS_WHO_CHARACTERS[this.secret].name,turns:this.turns};this.gamesCompleted+=1;if(won){this.gamesWon+=1;this.totalWinningTurns+=this.turns;this.bestTurns=this.bestTurns===null?this.turns:Math.min(this.bestTurns,this.turns);}}
  act(action,payload={}){if(action==="new_game"){this.start();return;}if(this.phase!=="playing")throw new Error("start a new Guess Who game first");if(action==="ask_question"){const index=GUESS_WHO_QUESTIONS.findIndex(question=>question.id===payload.questionId);if(index<0)throw new Error("unknown Guess Who question");const bit=1<<index;if(!(this.remaining&bit))throw new Error("that question has already been asked");const[yes,no]=guessWhoSplit(this.candidates,index);if(!yes||!no)throw new Error("that question no longer separates the remaining characters");const before=guessWhoCount(this.candidates),answer=Boolean(GUESS_WHO_YES_MASKS[index]&(1<<this.secret));this.candidates=answer?yes:no;this.remaining&=~bit;this.turns+=1;this.history.push({turn:this.turns,action:"question",questionId:payload.questionId,label:GUESS_WHO_QUESTIONS[index].label,answer,beforeCandidates:before,afterCandidates:guessWhoCount(this.candidates)});}else if(action==="guess_character"){const index=GUESS_WHO_CHARACTERS.findIndex(character=>character.name===payload.name);if(index<0)throw new Error("unknown Guess Who character");const bit=1<<index;if(!(this.candidates&bit))throw new Error("that character has already been eliminated");const before=guessWhoCount(this.candidates),correct=index===this.secret;this.turns+=1;if(!correct)this.candidates&=~bit;this.history.push({turn:this.turns,action:"guess",character:payload.name,correct,beforeCandidates:before,afterCandidates:guessWhoCount(this.candidates)});if(correct){this.candidates=bit;this.finish(true,"correct_guess");return;}}else throw new Error("unknown Guess Who action");if(this.turns>=this.maxTurns)this.finish(false,"turn_limit");}
...
class GoofspielSession {
  constructor(options={}){this.mode=String(options.mode||"basic").trim().toLowerCase();if(!["basic","advanced"].includes(this.mode))throw new Error("Goofspiel mode must be basic or advanced");this.start();}
  start(){this.prizeOrder=shuffle([1,2,3,4]);this.playerCards=[1,2,3,4];this.aiCards=[1,2,3,4];this.round=1;this.playerScore=0;this.aiScore=0;this.phase="bidding";this.history=[];}
  currentPrize(){return this.phase==="bidding"?this.prizeOrder[this.round-1]:null;}
  prizes(){return this.phase==="bidding"?[...this.prizeOrder.slice(this.round-1)].sort((a,b)=>a-b):[];}
  key(){return `${this.playerCards.join(",")}|${this.aiCards.join(",")}|${this.prizes().join(",")}|${this.currentPrize()}`;}
  policy(){const entry=GOOFSPIEL_POLICY[this.key()];if(!entry)throw new Error("missing exact Goofspiel policy state");return entry;}
  basicBid(prize){return[...this.aiCards].sort((a,b)=>Math.abs(a-prize)-Math.abs(b-prize)||a-b)[0];}
  sampleCard(cards,probabilities){let target=Math.random();for(let index=0;index<cards.length;index+=1){target-=probabilities[index];if(target<=0)return cards[index];}return cards.at(-1);}
  act(action,payload={}){if(action==="new_match"&&this.phase==="finished"){this.start();return;}if(action!=="bid"||this.phase!=="bidding")throw new Error("illegal Goofspiel action");const playerBid=Number(payload.card);if(!Number.isInteger(playerBid)||!this.playerCards.includes(playerBid))throw new Error("bid a card that remains in your hand");const prize=this.currentPrize(),policy=this.policy(),playerIndex=this.playerCards.indexOf(playerBid),aiBid=this.mode==="basic"?this.basicBid(prize):this.sampleCard(this.aiCards,policy.ai);let outcome="tie";if(playerBid>aiBid){outcome="player";this.playerScore+=prize;}else if(aiBid>playerBid){outcome="ai";this.aiScore+=prize;}this.history.push({round:this.round,prize,playerBid,aiBid,outcome,playerScore:this.playerScore,aiScore:this.aiScore,equilibriumValue:policy.value,aiDistribution:this.aiCards.map((card,index)=>({card,probability:this.mode==="basic"?Number(card===aiBid):policy.ai[index]})),aiPolicy:this.mode,playerDistribution:this.playerCards.map((card,index)=>({card,probability:policy.player[index]})),playerBidProbability:policy.player[playerIndex]});this.playerCards=this.playerCards.filter(card=>card!==playerBid);this.aiCards=this.aiCards.filter(card=>card!==aiBid);if(this.round===this.prizeOrder.length)this.phase="finished";else this.round+=1;}
  review(){if(this.phase!=="finished")return null;const probabilities=this.history.map(item=>item.playerBidProbability);return{scoreDifference:this.playerScore-this.aiScore,equilibriumSupportedRounds:probabilities.filter(value=>value>1e-9).length,averageChosenProbability:probabilities.reduce((a,b)=>a+b,0)/probabilities.length,offSupportRounds:this.history.filter(item=>item.playerBidProbability<=1e-9).map(item=>item.round),lowFrequencyRounds:this.history.filter(item=>item.playerBidProbability>0&&item.playerBidProbability<.1).map(item=>item.round)};}
  snapshot(){const policy=this.phase==="bidding"?this.policy():null,advisor=policy?this.playerCards.map((card,index)=>({card,probability:policy.player[index]})):[],recommended=advisor.length?[...advisor].sort((a,b)=>b.probability-a.probability||b.card-a.card)[0].card:null,winner=this.phase!=="finished"?null:this.playerScore>this.aiScore?"player":this.aiScore>this.playerScore?"ai":"tie";return{gameId:"goofspiel",mode:this.mode,phase:this.phase,roundNumber:this.round,roundsTotal:4,currentPrize:this.currentPrize(),playerCards:this.playerCards,aiCards:this.aiCards,playerScore:this.playerScore,aiScore:this.aiScore,history:this.history,lastRound:this.history.at(-1)||null,winner,postMatchReview:this.review(),advisorDistribution:advisor,recommendedBid:recommended,futureValue:policy?.value||0,legalActions:this.phase==="bidding"?["bid"]:["new_match"],strategyScope:this.mode==="basic"?"deterministic_match_prize_heuristic_four_card":"exact_four_card_shuffled_prize_zero_sum_equilibrium",strategyEvidence:this.mode==="basic"?"strong_heuristic":"equilibrium_backed",aiExploitability:this.mode==="basic"?2:0,equilibriumValue:0,informationSet:{currentPrize:this.currentPrize(),playerRemaining:this.playerCards,aiRemaining:this.aiCards,aiCurrentBidHidden:this.phase==="bidding",unrevealedPrizeCount:Math.max(0,this.prizes().length-1),publicHistory:this.history}};}
}

function createSession(gameId, options={}) {
  const factories={cases:CaseSession,worm:WormSession,pirates:PirateSession,"kuhn-poker":PokerSession,"e-card":ECardSession,"restricted-rps":RPSSession,blackjack:BlackjackSession,"liars-dice":LiarDiceSession,mastermind:MastermindSession,"guess-who":GuessWhoSession,battleship:BattleshipSession,"hidden-pursuit":HiddenPursuitSession,"love-letter":LoveLetterSession,investment:InvestmentSession,goofspiel:GoofspielSession};
...
async function api(request, url) {
  if (request.method === "GET" && url.pathname === "/api/health") return json({status:"ok",service:"aip-public",apiVersion:4});
