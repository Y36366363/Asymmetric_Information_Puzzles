...
const GAMES = [
  ...
  ["goofspiel", "秘密竞价", "奖牌逐轮揭晓，双方同时秘密打出唯一的竞价牌；用有限手牌对抗精确均衡 AI。", "单人 · 同时行动与秘密竞价", true],
...
class GoofspielSession {
  ...
  act(action,payload={}){if(action==="new_match"&&this.phase==="finished"){this.start();return;}if(action!=="bid"||this.phase!=="bidding")throw new Error("illegal Goofspiel action");const playerBid=Number(payload.card);if(!Number.isInteger(playerBid)||!this.playerCards.includes(playerBid))throw new Error("bid a card that remains in your hand");const prize=this.currentPrize(),policy=this.policy(),playerIndex=this.playerCards.indexOf(playerBid),aiBid=this.sampleCard(this.aiCards,policy.ai);let outcome="tie";if(playerBid>aiBid){outcome="player";this.playerScore+=prize;}else if(aiBid>playerBid){outcome="ai";this.aiScore+=prize;}this.history.push({round:this.round,prize,playerBid,aiBid,outcome,playerScore:this.playerScore,aiScore:this.aiScore,equilibriumValue:policy.value,aiDistribution:this.aiCards.map((card,index)=>({card,probability:policy.ai[index]})),playerDistribution:this.playerCards.map((card,index)=>({card,probability:policy.player[index]})),playerBidProbability:policy.player[playerIndex]});this.playerCards=this.playerCards.filter(card=>card!==playerBid);this.aiCards=this.aiCards.filter(card=>card!==aiBid);if(this.round===this.prizeOrder.length)this.phase="finished";else this.round+=1;}
