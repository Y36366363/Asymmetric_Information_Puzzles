...
class BattleshipSession {
  ...
  snapshot(){const density=battleDensity(this.advisor,this.size),suggestion=this.phase==="player_turn"?chooseBattleShot(this.advisor,this.size,false).cell:null,finished=this.phase==="finished";return{gameId:"battleship",phase:this.phase,turn:this.turn,winner:this.winner,boardSize:this.size,boardSizes:Object.keys(BATTLE_FLEETS).map(Number),shipLengths:this.lengths,fleet:this.player.ships.map((ship,id)=>({id,length:ship.length,orientation:battleOrientation(ship)})),playerBoard:this.boardPayload(this.player,true),enemyBoard:this.boardPayload(this.enemy,finished),playerShipsRemaining:this.remaining(this.player),enemyShipsRemaining:this.remaining(this.enemy),suggestedShot:suggestion,candidatePlacementCount:density.candidatePlacements,lastAiAnalysis:this.lastAiAnalysis,history:this.history,strategyScope:"cluster-consistent one-step placement density on 10x10/12x12; tail-protected legacy density on 15x15; not a full-fleet posterior optimum",legalActions:this.phase==="placement"?["randomize_fleet","set_board_size","rotate_ship","start_battle"]:this.phase==="player_turn"?["fire"]:[],informationSet:{misses:[...this.advisor.misses].map(battleCell),unresolvedHits:[...this.advisor.hits].map(battleCell),sunkCells:[...this.advisor.sunk].map(battleCell),remainingShipLengths:this.advisor.remaining,candidatePlacementCount:density.candidatePlacements}};}
}
