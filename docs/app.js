...
function renderECard() {
  ...
  const currentForecast = state.aiTimingForecast.find((item) => item.duel === state.duelNumber)?.probability ?? 0;
  const modeLabel = language === "zh"
    ? { uniform: "尚无习惯数据：均匀策略", chase_player_timing: "追逐你常用的位置", avoid_player_timing: "避开你常用的位置" }[state.aiPolicyMode]
    : { uniform: "No habit data: uniform policy", chase_player_timing: "Chasing your usual timing", avoid_player_timing: "Avoiding your usual timing" }[state.aiPolicyMode];
  $("#ecardInformation").textContent = state.phase === "finished"
    ? (language === "zh"
      ? `AI 本轮在你出牌前已承诺第 ${state.aiCommittedDuel} 次对决使用${cardNames[state.aiRole]}；结算前这个位置始终隐藏。`
      : `Before you played, the AI committed its ${cardNames[state.aiRole]} to duel ${state.aiCommittedDuel}; that position stayed hidden until settlement.`)
    : (language === "zh"
      ? `AI 还剩 ${state.informationSet.opponentCardsLeft} 张牌，可能出${possible}。它已提前承诺整轮顺序；根据公开策略，本次出现${cardNames[state.aiRole]}的条件概率为 ${(currentForecast * 100).toFixed(1)}%。`
      : `The AI has ${state.informationSet.opponentCardsLeft} cards and may play ${possible}. Its order is already committed; under the public policy, the conditional chance of ${cardNames[state.aiRole]} now is ${(currentForecast * 100).toFixed(1)}%.`);
  $("#ecardTimingForecast").innerHTML = `<strong>${language === "zh" ? `AI 时机模型 · ${modeLabel}` : `AI timing model · ${modeLabel}`}</strong>${state.aiTimingForecast.map((item) => `<div class="ecard-timing-row ${item.duel === state.duelNumber && state.phase === "playing" ? "current" : ""}"><span>${language === "zh" ? `第 ${item.duel} 次` : `Duel ${item.duel}`}</span><progress max="1" value="${item.probability}" aria-label="${language === "zh" ? `第 ${item.duel} 次概率` : `Duel ${item.duel} probability`}"></progress><b>${(item.probability * 100).toFixed(0)}%</b></div>`).join("")}`;
  const recentRounds = state.roundHistory.slice(-4).map((item) => language === "zh"
    ? `R${item.round}：AI 特殊牌第 ${item.aiCommittedDuel} 次${item.playerSpecialDuel == null ? "；你未出特殊牌" : `；你在第 ${item.playerSpecialDuel} 次出特殊牌`}`
    : `R${item.round}: AI special on ${item.aiCommittedDuel}${item.playerSpecialDuel == null ? "; yours was not played" : `; yours on ${item.playerSpecialDuel}`}`);
  $("#ecardTimingRecord").textContent = recentRounds.length
    ? `${language === "zh" ? "最近可观察记录" : "Recent observable record"} · ${recentRounds.join(language === "zh" ? " ｜ " : " | ")}`
    : (language === "zh" ? "完成一轮后，这里会保留双方特殊牌时机，供下一轮识别和反制。" : "After one round, both special-card timings remain here for cross-round inference and counterplay.");
  ...
  if (state.phase === "finished") {
    ...
    $("#ecardResult").innerHTML = `<strong>${won ? (language === "zh" ? "你赢得本轮" : "You win the round") : (language === "zh" ? "AI 赢得本轮" : "AI wins the round")} · +${state.result.points}</strong><p>${language === "zh" ? `${roleName(state.result.winnerRole)}在第 ${state.result.decisiveDuel} 次对决获胜。AI 开局预先承诺的特殊牌位置是第 ${state.result.aiCommittedDuel} 次。` : `${roleName(state.result.winnerRole)} prevailed on duel ${state.result.decisiveDuel}. The AI had precommitted its special card to duel ${state.result.aiCommittedDuel}.`}</p><button data-ecard-next>${language === "zh" ? "交换阵营，进入下一轮" : "Swap sides and play next round"}</button>`;
