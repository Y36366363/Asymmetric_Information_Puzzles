...
class LocalGameUITests(unittest.TestCase):
    ...
    def test_lobby_assets_include_language_and_github_navigation(self) -> None:
        ...
        self.assertIn('button.disabled = pending', script)
        self.assertIn('$("#pokerModeSwitch").setAttribute("aria-label"', script)
        self.assertIn('$("#goofModeSwitch").setAttribute("aria-label"', script)
        self.assertIn('pokerMode = currentState.mode;\n      writePreference', script)
        self.assertIn('goofspielMode = currentState.mode;\n      writePreference', script)
        self.assertNotIn('pokerMode = "advanced";', script)
        self.assertNotIn('goofspielMode = "advanced";', script)
        ...
        self.assertIn('function renderModeContract(selector, details)', script)
        self.assertIn('changes: "改变内容"', script)
        self.assertIn('applies: "生效时机"', script)
        self.assertIn('score: "成绩处理"', script)
        self.assertIn('Selecting it immediately starts a new match', script)
        self.assertIn('Normal decisions do not enter Practice accuracy', script)
        self.assertIn("最佳回应可达 +1/6 筹码/局", script)
